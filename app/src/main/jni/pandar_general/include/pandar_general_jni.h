/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode */

#ifndef _Included_org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode
#define _Included_org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode
#ifdef __cplusplus
extern "C" {
#endif
#undef org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode_SUCCESS
#define org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode_SUCCESS 0L
/*
 * Class:     org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode
 * Method:    execute
 * Signature: (Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;)I
 */
JNIEXPORT jint JNICALL Java_org_ros_rosjava_1tutorial_1native_1node_PandarGeneralNativeNode_execute
  (JNIEnv *, jobject, jstring, jstring, jstring, jobjectArray);

/*
 * Class:     org_ros_rosjava_tutorial_native_node_PandarGeneralNativeNode
 * Method:    shutdown
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_org_ros_rosjava_1tutorial_1native_1node_PandarGeneralNativeNode_shutdown
  (JNIEnv *, jobject);

#ifdef __cplusplus
}
#endif
#endif
